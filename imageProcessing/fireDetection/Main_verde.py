...
def getGreenKMeans(imgBGR):
    ...
    # Usa limiarização padro (valor de 30 ficou bom) para binarizar o canal específico
    thG3 = imgG > 30
    binGreen = thG3
    binGreen = 255*(binGreen).astype(np.uint8)
    # Cria uma mascar usando a imagem binaria citada anteriormente
    imgGreen = cv2.bitwise_and(img, img, mask=binGreen)
    
    # Seleciona quais canais do espaço de cor serão usados para a segmentação
    # Nesse caso será usado o espaço de cor HSV, utilizando apenas o V (separa por luminosidade os dois tons de verde)
    canais = [2]

    #Transformação
    tst = cv2.cvtColor(imgGreen, cv2.COLOR_RGB2HSV)
    ...
    Para o caso do Verde Escuro verificamos se o valor de V está dentro da faixa 2 < V < 195,
    pois essa faixa de cor representa as cores mais próximas do verde escuro.
    ...
    # Faz uma abertura, pois estava com muitos ruidos pequenos nessa imagem
    imgFinal1 = ndi.binary_opening(imgFinal1, iterations=3)
    ...
    '''
    Nessa parte so definidas as regras que selecionam quais cluster vão reprensentar a cor
    Para o caso do Verde  verificamos se o valor de V está dentro da faixa 2 < V < 195,
    pois essa faixa de cor representa as cores mais próximas do verde claro.
    Os clusters que tiverem seu valor dentro dos parametros são adicionados na lista chosenColors
    '''
    chosenColors = []
    for i,cent in enumerate(centers):
    ...
    #imgFinal = ndi.binary_opening(imgFinal, iterations=1)
    #Cria a mascara final a ser retornada
...
while running:
    ...
    if calc:
        ClaroMask,EscuroMask=getGreenKMeans(img)
        imgClaro=cv2.bitwise_and(img, img, mask=ClaroMask)
        imgEscuro=cv2.bitwise_and(img, img, mask=EscuroMask)
    ...
    cv2.imshow('Claro',cv2.resize(imgClaro,(int(1.5*img.shape[1]),int(1.5*img.shape[0])),interpolation=cv2.INTER_NEAREST))
    cv2.moveWindow('Claro',64,32)

    cv2.imshow('Escuro',cv2.resize(imgEscuro,(int(1.5*img.shape[1]),int(1.5*img.shape[0])),interpolation=cv2.INTER_NEAREST))
    cv2.moveWindow('Escuro',64,32)
