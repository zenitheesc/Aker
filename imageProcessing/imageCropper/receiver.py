...
class LoRaRcvCont(LoRa):
    ...
    var = 0
    missed_packets = []
    # If zero, will receive only one of each byte
    # If one, will receive three of each byte and vote
    rcv_repeatead = 0
        ...
        if payload[0] == self.var: 
            ...
            # Voting rcv
                ...
                for i in range(1, len(payload)):
                    ...
                    # Creates an array to store each three bytes
                    repeated_bytes = bytearray(3)
                    
                    # Store each three bytes
                    for j in range(0, 3):
                        repeated_bytes.append(payload[i*3+j])
                    
                    print repeated_bytes

                    # The buffer will receive the most common byte of the repeated bytes
                    self.receiver_buffer[i] = max(set(repeated_bytes), key=repeated_bytes.count)
