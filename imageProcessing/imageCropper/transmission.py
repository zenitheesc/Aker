...
class LoRaBeacon(LoRa):
	...
	resto = 0
	first_time = 1
	# If zero, send each byte once
	# If one, send each byte three times
	send_repeated = 0 
	...
	def send_image(self):     
		...
		if self.sending == 1:
			...
			if self.iteration < self.number_packets:
				...
				elif send_repeated == 1:
					# Appending three times the same byte value
					Z.append(self.image_buffer[self.iteration*self.step])
					Z.append(self.image_buffer[self.iteration*self.step])
					Z.append(self.image_buffer[self.iteration*self.step])
